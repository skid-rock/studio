...
- `source/base/` — общие DTCG-токены (типографика, размеры, тени, motion).
...
с CSS-переменными (`--color-navy`, `--font-display`, `--shadow-photo`, …).

## Граница «тема vs инстанс»

В токены темы попадают только примитивы (палитра, типографика, тени, отступы,
радиусы, длительности/easing). Параметры `--envelope-*` и `--slot-*` —
per-instance props модулей, не часть темы.
