...
describe('documentToPuck', () => {
    it('маппит секции в Puck Data с экранированным type и props.id', () => {
        ...
        expect(data.content.every((c) => typeof c.props.id === 'string')).toBe(
            true,
        );
    });
});
...
describe('puckToDocument round-trip', () => {
    it('вставка в середину сохраняет ключи соседей', () => {
        ...
        const inserted: Data = {
            ...
            content: [
                intro,
                hero,
                { type: toPuckType('hero'), props: { id: 's_new' } },
                closing,
            ],
        };
        ...
        const byId = Object.fromEntries(
            doc1.sections.map((s) => [s.id, s.order]),
        );
    ...
    it('сохраняет правки, порядок и дробный order после edit/reorder/add', () => {
        ...
        expect(orders.every((o, i) => i === 0 || orders[i - 1]! < o)).toBe(
            true,
        );

        expect(doc1.schemaVersion).toBe(1);
        expect(doc1.theme.id).toBe('cream-navy');
        expect(sorted.filter((s) => s.type === 'intro/envelope')).toHaveLength(
            2,
        );
        expect(sorted.find((s) => s.id === 's_hero')?.props.names).toBe(
            'Аня & Боря',
        );
    });
});
...
describe('renderDocument после round-trip', () => {
    it('даёт строковый HTML без React-рантайма и с отредактированным контентом', () => {
        ...
        expect(/data-reactroot|__reactProps\$|reactFiber/i.test(out.html)).toBe(
            false,
        );
...
describe('samePuckProps', () => {
    ...
    it('разное значение поля → false', () => {
        expect(
            samePuckProps({ names: 'Аня & Боря' }, { names: 'Полина & Илья' }),
        ).toBe(false);
    });
    ...
    it('разный набор ключей → false', () => {
        expect(
            samePuckProps(
                { eyebrow: 'Мы женимся' },
                { eyebrow: 'Мы женимся', date: '05.08.2026' },
            ),
        ).toBe(false);
    });
});
