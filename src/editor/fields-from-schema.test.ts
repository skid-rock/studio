...
describe("fieldsFromSchema", () => {
  ...
  it("text → textarea", () => {
    const schema: ParamSchema = [
      {
        group: "Текст",
        items: [{ key: "names", label: "Имена", type: "text", def: "" }],
      },
    ];

    expect(fieldsFromSchema(schema).names).toEqual({ type: "textarea", label: "Имена" });
  });
  ...
  it("select → select с options", () => {
    ...
    const schema: ParamSchema = [
      {
        group: "Лейаут",
        items: [{ key: "align", label: "Выравнивание", type: "select", options, def: "center" }],
      },
    ];

    expect(fieldsFromSchema(schema).align).toEqual({ type: "select", label: "Выравнивание", options });
  });
  ...
  it("color → custom с render", () => {
    const schema: ParamSchema = [
      {
        group: "Цвета",
        items: [{ key: "lineColor", label: "Цвет линии", type: "color", def: "#275889" }],
      },
    ];

    const field = fieldsFromSchema(schema).lineColor;
    expect(field.type).toBe("custom");
    if (field.type !== "custom") throw new Error("ожидалось custom-поле");
  ...
  it("плющит группы в плоский Record по key", () => {
    const schema: ParamSchema = [
      ...
      },
      {
        group: "B",
        items: [
          { key: "b", label: "B", min: 0, max: 1, step: 0.1, def: 0.5, unit: "" },
