...
export function sortedSections(doc: StudioDocument): SectionNode[] {
  return [...doc.sections].sort((a, b) => (a.order < b.order ? -1 : a.order > b.order ? 1 : 0));
}
...
export function moveSection(doc: StudioDocument, id: string, toIndex: number): StudioDocument {
  const list = sortedSections(doc);
  ...
  return {
    ...doc,
    sections: doc.sections.map((section) => (section.id === id ? { ...section, order } : section)),
  };
}

/** Удалить секцию по id. */
export function removeSection(doc: StudioDocument, id: string): StudioDocument {
  return { ...doc, sections: doc.sections.filter((section) => section.id !== id) };
}
