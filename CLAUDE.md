...
- **Рекомендованная модель** — в шапке-маркере плана обязательна строка
  `Рекомендованная модель: <модель> — <почему хватает>`: какого уровня нейромодели
  достаточно для буквального исполнения. Указывается конкретная модель/вилка под уровень
  (лёгкая — Claude Haiku / GPT-5 mini; средняя — Claude Sonnet / GPT-5; сильная —
  Claude Opus / GPT-5 Codex). Критерии выбора (объём · риск · неоднозначность) и рубрика —
  в каноне (`gd-brain/tasks/README.md`, раздел «Планы реализации»).
- **Канон** формата и процесса — `gd-brain/tasks/README.md`, разделы «Workflow» и
  «Планы реализации» (там же обязательная шапка-маркер плана и рубрика выбора модели).
