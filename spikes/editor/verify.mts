...
check('type экранирован (нет "/")', data0.content.every((c) => !c.type.includes("/")));
check("у каждой секции есть props.id", data0.content.every((c) => typeof c.props.id === "string"));
...
check(
  ...
  sorted.map((s) => s.id).join(",") === added.content.map((c) => c.props.id).join(","),
);
...
check("schemaVersion/theme сохранены из base", doc1.schemaVersion === 1 && doc1.theme.id === "cream-navy");
check(
...
check(
  ...
  (sorted.find((s) => s.id === "s_hero")?.props.names as string) === "Аня & Боря",
);
...
check("вывод содержит разметку конверта", out.html.includes("envelope-overlay"));
...
check("обёртка содержит ровно строковый HTML render", reactHtml.includes(agnostic));
check("обёртка не добавила своего markup в блок", toStudioType("intro--envelope") === "intro/envelope");
