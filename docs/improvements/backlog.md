...
- **Статус:** открыто, **триггер сработал в STUDIO-063** — появился второй
  потребитель `drop-shadow` вне envelope.
- **Где:** [`src/sections/intro-envelope/styles.ts:89`](../../src/sections/intro-envelope/styles.ts)
  ...
  (`filter: drop-shadow(...)`); [`src/sections/deco-collage/index.ts:111`](../../src/sections/deco-collage/index.ts)
  (жемчужины коллажа).
...
Триггер сработал: в STUDIO-063 жемчужины `deco-collage` получили
`filter: drop-shadow(...)`, вручную повторяющий четыре слоя `--shadow-photo`.
Причина — форма: `box-shadow` рисует тень по прямоугольнику бокса, а жемчужина
круглая, и тень читается квадратом; `drop-shadow` идёт по альфе PNG.

Отличие от конверта: у envelope литералы **свои** (navy), у коллажа —
**дубликат существующего токена** в другой форме записи. Значит вопрос не «нужен
ли токен», а «одно значение — две формы»: `--shadow-photo` как `box-shadow` и он
же цепочкой `drop-shadow()`. Разойдутся при первой правке тени в теме.

Что рассмотреть: парный токен-фильтр (`--shadow-photo-filter`) рядом с
`--shadow-photo`, генерируемый из того же DTCG-источника — тогда обе формы
правятся в одном месте. Триггер — третий потребитель или первая правка значений
`shadow.photo` в теме.
...
## IMP-010. `make export` умеет ровно один документ — `landing.sample.json`

- **Статус:** открыто (обнаружено в STUDIO-063).
- **Где:** [`scripts/export.mts:24`](../../scripts/export.mts) — путь к документу
  зашит в `readFileSync`.
- **Появилось в:** STUDIO-018 (статический экспорт), проявилось с приходом
  второго документа-примера.

### Симптом

`make export` всегда рендерит `examples/landing.sample.json` (Фаза 0, 10 секций).
Проверить экспортом документ фазы B (`examples/landing.phase1.json`) нечем: в
STUDIO-063 пункт DoD «прогнать `make export`» пришлось закрывать одноразовым
скриптом поверх `renderDocument`. Каждая следующая секция морского лендинга
упрётся в то же место.

### Корень

Скрипт писался под единственный пример и берёт путь константой, хотя всё
остальное — реестр, `renderDocument`, сборка CSS — от документа не зависит.

### Рекомендация и триггер

Принять путь аргументом (`npm run export -- examples/landing.phase1.json`) со
старым значением по умолчанию, каталог вывода — рядом (`--out`). Правка
локальная, ломать нечего: вызов без аргументов ведёт себя как сейчас.

Триггер — уже сработал: документов-примеров больше одного. Делать при первой
задаче, которой снова понадобится экспорт фазы B.

---

## IMP-011. Live-JS секций не запускается в холсте редактора

- **Статус:** открыто (обнаружено в STUDIO-064).
- **Где:** [`src/editor/section-scripts.ts:33`](../../src/editor/section-scripts.ts)
  — `runModuleJs`; вызов — [`src/editor/EditorOwn.tsx:67`](../../src/editor/EditorOwn.tsx)
  (`useEffect` + `requestAnimationFrame`, зависимость `[state.document]`).
- **Появилось в:** не установлено. На STUDIO-064 проверено, что баг старше задачи:
  поведение одинаково и на новом модуле `countdown`, и на доноре до правок
  (сверка через `git stash` на том же dev-сервере).

### Симптом

В превью редактора клиентский тик `countdown` не привязывается: ни на одной
секции нет `__cdTimer`, цифры остаются плейсхолдерами из разметки («00»). Смена
целевой даты в панели свойств делу не помогает — новый `data-countdown-target`
в DOM приезжает, а отсчёт не идёт.

Важно, что механика сама по себе исправна: в экспортированном HTML тот же
скрипт работает (проверено на сборке `make export` с будущей датой — цифры
тикают, повторного таймера не возникает), а юнит-тест бриджа
`src/editor/section-scripts.test.ts` зелёный — там `runModuleJs` зовут напрямую
с готовым корнем на happy-dom. То есть дыра ровно между бриджем и живым холстом.

### Корень

Не установлен. Что известно: `.ch-cv-page` в DOM присутствует, разметка секции
внутри него, ошибок в консоли браузера нет. Рабочие гипотезы — прогон в `rAF`
случается раньше, чем React вставляет разметку блоков (`BlockPreview`), а
повторной попытки нет, потому что эффект завязан только на `state.document`;
либо корень эффекта и корень разметки расходятся во времени монтирования.
Проверять надо в браузере, юнит-тест этот разрыв не ловит по построению.

### Рекомендация и триггер

Разобраться до правки: залогировать в `runModuleJs` факт вызова и число
найденных `[data-countdown-root]`, сравнить с моментом появления разметки.
Дальше по результату — либо перевесить прогон на реальное появление узлов
(`MutationObserver` / колбэк после вставки разметки в `BlockPreview`), либо
дать бриджу вторую попытку. Заодно завести проверку, которая ловит разрыв не на
happy-dom, а на живом холсте.

Триггер — первая же секция с клиентским JS, которую надо принять «как в макете»
глазами в редакторе (сейчас таких две: `countdown` и `rsvp`), либо жалоба на
«отсчёт в редакторе стоит».
